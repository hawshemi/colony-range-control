...
- Before changing game hooks, check the installed game source. On this machine it is at `E:/SM/ModTools/Src`.
...
```powershell
python tests/test_ranges.py mod --game-source "E:/SM/ModTools/Src"
python tests/test_heat.py --game-source "E:/SM/ModTools/Src"
python tests/test_release.py mod --game-source "E:/SM/ModTools/Src"
```

Use the first test for range logic, the second for heat and forestation behavior, and the third for metadata, options, and thumbnail changes. Adjust the game source path on other machines. These tests simulate engine services and do not prove in-game behavior. Report what was actually checked.
